"""Master flare catalogue: SoLEXS and HEL1OS detected independently, merged, checked against GOES.
    ...
    python scripts/master_catalog.py --cache-dir outputs/archive/cache --goes-dir D:/Data/goes
...
4. Validate against the GOES-18 flare list (never used for detection):
   ...
   again on the test period alone (2026-03-24 onwards), which the flux
   calibration never saw. The rule run on GOES's own flux gives the ceiling.
...
from solarflare.config import PreprocessConfig  # noqa: E402
from solarflare.io.goes import load_goes  # noqa: E402
from solarflare.preprocess.cache import load_cached  # noqa: E402
from solarflare.preprocess.timeline import stitch  # noqa: E402

DT = 20.0
TEST_START = datetime(2026, 3, 24, tzinfo=UTC).timestamp()
#: End of the training period of the GOES-labelled run (data_meta.json split_dates.train_end).
...
#: Hours that four PRADAN day files copy from the previous day (scripts/solexs_duplicates.py).
DUPLICATES = Path(__file__).resolve().parents[1] / "outputs" / "catalog" / "solexs_duplicates.json"
...
def main() -> int:
    ...
    ap.add_argument("--out", default="outputs/catalog")
...
def render(s: dict) -> str:
    ...
    L.append(f"Generated {s['generated_utc']} UTC by `scripts/master_catalog.py`. Period {s['period'][0]} → "
