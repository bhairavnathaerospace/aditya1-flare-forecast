...
(outputs/hxr/hxr_spectra.csv, from scripts/hxr_spectra.py):
...
Writes outputs/hxr/{subsecond.csv, subsecond_summary.json, SUBSECOND.md, subsecond.png}.
"""
...
from solarflare.io.hel1os_events import TICK_S, product_for, read_events, tick_counts  # noqa: E402
...
def render(s: dict) -> str:
    L = ["# HEL1OS timing audit and sub-second hard X-rays", "",
         f"Generated {s['generated_utc']} UTC by `scripts/hxr_timing.py` on {s['flares']} flares with a reliable CZT "
    ...
    if s.get("scales"):
        ...
        for k, v in s["scales"].items():
            def c(key):
                return (f"{v[key + '_median']:+.4f} ({100 * v[key + '_above_zero']:.0f}%)"
                        if v.get(key + "_median") is not None else "--")
...
def main() -> int:
    ...
    ap.add_argument("--spectra", default="outputs/hxr/hxr_spectra.csv")
    ap.add_argument("--hel1os-root", default="D:/Data/extracted/hel1os")
    ap.add_argument("--out", default="outputs/hxr")
    args = ap.parse_args()
    sys.stdout.reconfigure(encoding="utf-8")
    out = Path(args.out)
    rows = [r for r in csv.DictReader(open(args.spectra, encoding="utf-8")) if r.get("status") == "ok"]
    t_start = time.time()
    res = []
    for r in rows:
