...
Writes outputs/hxr/{hxr_spectra.csv, hxr_timeresolved.csv, hxr_summary.json, HXR.md}.
"""
...
def main() -> int:
    ...
    rows = [r for r in csv.DictReader(open(args.catalog, encoding="utf-8")) if r["hard_peak_czt_20_40_utc"]]
...
def render(s: dict) -> str:
    ...
    L = ["# Hard X-ray spectral index from HEL1OS photon lists", "",
         f"Generated {s['generated_utc']} UTC by `scripts/hxr_spectra.py`. {s['flares_with_czt_burst']} flares have a "
    ...
    L += ["## Caveats", "",
          ...
          "- HEL1OS reads events out in batches (scripts/hxr_timing.py): below ~2,000 events/s photons are displaced "
