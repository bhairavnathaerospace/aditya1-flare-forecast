"""Flare plasma temperature from SoLEXS spectra, checked against GOES and HEL1OS CdTe.
    ...
    python scripts/solexs_temperature.py                 # all SoLEXS flares >= C5 (~1 h)
    python scripts/solexs_temperature.py --min-flux 1e-5 # M and X only
...
Writes outputs/temperature/{flare_temperatures.csv, temperature_bins.csv,
...
from solarflare.io.goes import class_flux  # noqa: E402
from solarflare.io.hel1os_events import product_for, read_events  # noqa: E402
from solarflare.io.solexs import channel_energies, read_solexs_zip  # noqa: E402
from solarflare.physics.onset import load_goes_xrs  # noqa: E402
...
def main() -> int:
    ...
    ap.add_argument("--hel1os-root", default="D:/Data/extracted/hel1os")
    ap.add_argument("--min-flux", type=float, default=5e-6, help="SoLEXS-derived GOES peak flux, W/m^2")
    ap.add_argument("--out", default="outputs/temperature")
    args = ap.parse_args()
    ...
    rows = [r for r in csv.DictReader(open(args.catalog, encoding="utf-8"))
    ...
    flares, bins = [], []
    for n_day, (day, rs) in enumerate(sorted(by_day.items())):
        zp = zips.get(day)
        if zp is None:
            continue
...
def render(s: dict) -> str:
    L = ["# Flare temperatures from SoLEXS spectra", "",
         f"Generated {s['generated_utc']} UTC by `scripts/solexs_temperature.py`: {s['fitted']} of {s['flares']} SoLEXS "
    ...
    if b:
        ...
        t = s["T_peak_vs_class"]
        L += ["", f"Bigger flares are hotter: Spearman {t['spearman']} between peak temperature and peak flux, "
              f"{t['slope_MK_per_decade']} MK per decade of flux. The plasma is hottest before the flux peaks, as the "
              "hot-onset study found.", ""]
    L += ["## Checks", ""]
    ...
    L += ["", "## Caveats", "",
          ...
          "spectrum and pushes T up; the 30-43 MK peaks of the X3-X5 flares should be read with that in mind.",
