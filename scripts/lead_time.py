"""Lead time per flare: how long before the GOES peak an alert fires, and at what false-alarm cost.
    ...
    python scripts/lead_time.py --predict                 # once, GPU ~13 min: frozen v3 at a 1-min stride
    python scripts/lead_time.py --predict --blank-hard    # the same with HEL1OS hidden (paired ablation)
    python scripts/lead_time.py                           # scoring + figure, ~1 min
...
model's origin bin ends 20 s before A; a SoLEXS minute [A-60, A) ends at A).
  ...
  network (C)  v3 P(a GOES >= C1 flare is in progress within 15 min)
  network (M)  v3 median forecast of GOES flux, highest of +5/+15/+30 min
...
Operating points are chosen on the validation period (2025-09-07 -> 2026-03-24)
and only scored on the test period: (a) the best-TSS threshold on minute labels
(the criterion the frozen v3 thresholds used; v3's own C threshold is the frozen
one), and (b) the threshold giving a set number of false alarms per day, so
every method is compared at the same false-alarm cost.
...
Writes outputs/leadtime/{lead_times.csv, leadtime_summary.json, LEADTIME.md, leadtime.png}.
"""
...
TRAIN_END = 1757237860.0              # data_meta.json split_dates.train_end
TEST_START = 1774325020.0             # data_meta.json split_dates.test_start
EMBARGO_S = 3600.0
PRE_START_MIN = 30
MERGE_GAP_MIN = 5
SHIFT_MIN = 120
...
NAMES = {"model": "v3 network", "model_no_hel1os": "v3, HEL1OS hidden",
    ...
    with torch.no_grad():
        for seg in segments:
            ...
            for b0 in range(0, ends.size, args.batch):
                ...
                rec["p_occ"].append(torch.sigmoid(o["occurrence"]).cpu().numpy())
...
def score(args) -> int:
    ...
    cat = list(csv.DictReader((Path(args.catalog) / "master_catalog.csv").open(encoding="utf-8")))
    pairs = [(ts_(x["soft_alert_utc"]) + 60.0, ts_(x["peak_utc"]) + 60.0) for x in cat if x["soft_alert_utc"]]
    ...
    methods = {"C": ["model", "trend", "current", "rule", "rule_C_level"], "M": ["model", "trend", "current"]}
    ...
    for c in ("C", "M"):
        ...
        if PH is not None:
            ...
            summary["results"][c]["hel1os_ablation"] = {
                ...
                "mean_lead_gain_ci": day_bootstrap(Wh["day"], lambda i: np.mean(np.nan_to_num(lw[i])) - np.mean(np.nan_to_num(lo_[i]))),
                "warned_gain_ci": day_bootstrap(Wh["day"], lambda i: np.mean(ww[i]) - np.mean(wo[i])),
            ...
            summary["results"][c]["hel1os_ablation"]["matched_false_alarms"] = {
                ...
                "warned_gain_ci": day_bootstrap(Wh["day"], lambda i: np.mean(ww[i]) - np.mean(wm_[i])),
                "mean_lead_gain_min": _r(np.mean(np.nan_to_num(lw)) - np.mean(np.nan_to_num(lm_)), 2),
                "mean_lead_gain_ci": day_bootstrap(Wh["day"], lambda i: np.mean(np.nan_to_num(lw[i])) - np.mean(np.nan_to_num(lm_[i]))),
            }
...
def render(s: dict) -> str:
    ...
    L = [f"# Lead time before the flare peak ({s['model']})", "",
         f"Generated {s['generated_utc']} UTC by `scripts/lead_time.py`. Test period {s['test_period'][0]} -> "
    ...
    for c, title in (("C", "Alert: a GOES >= C1 flare within 15 min"), ("M", "Alert: GOES flux will reach M1 within 30 min")):
        ...
        if c == "M":
            L += ["Before GOES first reached M1.0:", "", "| method | alert before M1 | median lead, min | lead >= 5 min |",
                  "|---|---|---|---|"]
            for mth in ("model", "trend", "current"):
...
def figure(s: dict, dest: Path) -> None:
    ...
    col = {"model": "#2a8c7c", "trend": "#d9822b", "current": "#4a6fa5"}
...
def main() -> int:
    ...
    ap.add_argument("--forward-dir", default="outputs/archive_goes_anchor/forward/v3")
    ap.add_argument("--run-dir", default="outputs/archive_goes_anchor")
    ...
    ap.add_argument("--catalog", default="outputs/catalog")
    ap.add_argument("--out", default="outputs/leadtime")
